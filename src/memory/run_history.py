...
class RunHistoryManager:
    ...
    def _write_history(self, runs: List[RunRecord]) -> None:
        ...
        try:
            ...
            if temp_file.exists():
                temp_file.unlink()
            raise IOError(f"Failed to write run history: {e}")
    ...
    def get_runs_by_query(self, query_text: str, case_sensitive: bool = False) -> List[RunRecord]:
        """
        ...
        with self._file_lock():
            ...
            if not case_sensitive:
                ...
                return [
                    run for run in runs
                    if query_text in run.query.lower()
                ]
            else:
                return [
                    run for run in runs
                    if query_text in run.query
                ]
    ...
    def get_runs_by_technique(self, technique: str) -> List[RunRecord]:
        ...
        with self._file_lock():
            ...
            return [
                run for run in runs
                if technique in run.enabled_techniques
            ]
...
def get_run_history_manager(
    history_file: str = "data/run_history.json",
    max_runs: int = 8
) -> RunHistoryManager:
    """
